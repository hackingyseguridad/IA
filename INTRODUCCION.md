


### La lista modelos de lenguaje de datos (LLM):

| Compañía | IA Modelo |
|----------|-----------|
| OpenAI | GPT |
| Google | Gemini |
| Anthropic | Claude |
| Meta | Llama |
| Microsoft | Copilot |
| x | Grok |
| Mistral AI | Mistral |
| Alibaba (China) | Qwen |
| Baidu (China) | ERNIE |
| ByteDance (China) | Doubao |
| DeepSeek (China) | DeepSeek |
| Moonshot AI (China) | Kimi |
| Zhipu AI (China) | GLM |
| Xiaomi (China) | MiMo |
| MiniMax (China) | MiniMax |
| iFLYTEK (China) | Spark Desk |
| Perplexity AI | Perplexity Pages |
| ElevenLabs | Eleven Multilingual |
| Midjourney | Midjourney |

### Privacidad de los datos que sube el usuario:

1º.- **MODELO NUBE ONLINE** modelos LLM desde un formulario Web (Chatboot) en internet o consola (Cli), los datos del usuario se suben a la nube y la procesa la compañia de la IA en centros de datos formados por granjas de GPUs de gran potencia con gran agilidad de proceso.

2º.- **MODELO LOCAL PC** modelo LLM instalado en localhost: por API/web (Chatboot) o por terminal (Cli) , procesa la inforamción en la CPU/GPU del sistema local sin subir la informacón del usuario a la nube a la compañia de la IA - solo  para autenticar y facturacion tokens consumidos.













